...
const main_layer = 0
const normal_wall_atlas_coords = Vector2i(6, 6)
const walkable_atlas_coords = Vector2i(28, 6)
const SOURCE_ID = 0
var spot_to_letter = {}
var spot_to_label = {}
var current_letter_num = 65
const label = preload("res://Maze/simple_label.tscn")
...
func _ready() -> void:
	y_dim = Global.grid_size
	x_dim = Global.grid_size
	Global.letters_to_show.clear()
	place_border()
	dfs(starting_coords)
	set_cell(0, Vector2i(0,-1), 0, walkable_atlas_coords, 0)
...
func place_wall(pos: Vector2):
	set_cell(main_layer, pos, SOURCE_ID, normal_wall_atlas_coords, 0)
...
func dfs(start: Vector2i):
	...
	while fringe.size() > 0:
		...
		Global.letters_to_show.pop_front()
		if current in seen or not can_move_to(current):
			if Global.show_labels and Global.step_delay > 0:
				await get_tree().create_timer(Global.step_delay).timeout
			continue
