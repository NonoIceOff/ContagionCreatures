...
func init_save_names():
	...
	for i in range(1, 4):
		...
		if dir.change_dir(folder_path) == OK:  # Si le dossier existe
			get_node("VBoxContainer/Fichier" + str(i) + "/RichTextLabel").text = "[wave][color=black]FICHIER " + str(i) + "[/color][/wave]"
		else:
			print("non")
			get_node("VBoxContainer/Fichier" + str(i) + "/RichTextLabel").text = "[wave][color=red]FICHIER " + str(i) + " NON TROUVÉ[/color][/wave]"
